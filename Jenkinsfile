node {
    ...
    stage ('Configure') {
        ...
        //sleep 30
        sh 'docker exec -ti JHG_wordpress sh -c "wp core install --url=http://localhost:8080/var/www/wordpress/ --title=Example --admin_user=supervisor --admin_password=strongpassword --admin_email=info@example.com --allow-root"'
